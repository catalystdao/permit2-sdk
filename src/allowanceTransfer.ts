...
import { TypedDataDomain, TypedDataField, BigNumberish, TypedDataEncoder } from 'ethers'
...
export abstract class AllowanceTransfer {
  ...
  public static getPermitData(
    ...
    invariant(BigInt(MaxSigDeadline) >= BigInt(permit.sigDeadline), 'SIG_DEADLINE_OUT_OF_RANGE')
...
function validatePermitDetails(details: PermitDetails) {
  invariant(BigInt(MaxOrderedNonce) >= BigInt(details.nonce), 'NONCE_OUT_OF_RANGE')
  invariant(BigInt(MaxAllowanceTransferAmount) >= BigInt(details.amount), 'AMOUNT_OUT_OF_RANGE')
  invariant(BigInt(MaxAllowanceExpiration) >= BigInt(details.expiration), 'EXPIRATION_OUT_OF_RANGE')
}
