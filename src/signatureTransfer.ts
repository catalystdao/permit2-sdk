...
export abstract class SignatureTransfer {
  ...
  public static getPermitData(
    ...
    invariant(BigInt(MaxSigDeadline) >= BigInt(permit.deadline), 'SIG_DEADLINE_OUT_OF_RANGE')
    invariant(BigInt(MaxUnorderedNonce) >= BigInt(permit.nonce), 'NONCE_OUT_OF_RANGE')
...
function validateTokenPermissions(permissions: TokenPermissions) {
  invariant(BigInt(MaxSignatureTransferAmount) >= BigInt(permissions.amount), 'AMOUNT_OUT_OF_RANGE')
}
